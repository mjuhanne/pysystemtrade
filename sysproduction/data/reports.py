...
class dataReports(productionDataLayerGeneric):
    def get_report_configs_to_run(self) -> dict:
        ...
        report_config = \
            populate_reporting_config_from_yaml_input(config_dict_from_yaml=config_dict_from_yaml,
...
def populate_reporting_config_from_yaml_input(config_dict_from_yaml: dict,
    ...
    new_config = dict([
        (report_name,
         _resolve_config_for_named_report(report_name=report_name,
...
def _resolve_config_for_named_report(report_name: str,
                                    config_dict_from_yaml: dict,
                                    default_config: dict
                                    )-> reportConfig:
    ...
    resolved_config = _resolve_config_from_config_pair(default_config_for_report,
                                                       new_config_for_report=new_config_for_report)

    return resolved_config

def _resolve_config_from_config_pair(default_config_for_report: reportConfig,
                                     new_config_for_report: dict) -> reportConfig:
    ...
    for attribute in attr_names:
        setattr(new_config,
                attribute,
                new_config_for_report[attribute])
